...
def ths_sga_required(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        if "ths_sga" not in formularios and session["usuario"].get("rol_real") != "admin" and session["usuario"].get("rol") != "admin":
            flash("No tienes permisos para acceder a la Documentación THS SGA.", "error")
...
def register_routes(app):
    ...
    def admin_ths_sga():
        ...
        conn.close()
        return render_template("ths_sga.html", activos=activos, inactivos=inactivos, is_superadmin=is_superadmin, usuario=session["usuario"])

    @app.route("/admin/ths_sga/agregar", methods=["POST"])
    @login_required
    @ths_sga_required
    def admin_ths_sga_agregar():
    ...
    @app.route("/admin/ths_sga/eliminar/<string:identificacion>", methods=["POST"])
    @login_required
    @ths_sga_required
    def admin_ths_sga_eliminar(identificacion):
        if identificacion:
            try:
                conn = get_db()
                conn.execute("DELETE FROM ths_sga_records WHERE identificacion = ?", (identificacion,))
                conn.commit()
                conn.close()
    ...
    @app.route("/admin/ths_sga/toggle/<string:identificacion>", methods=["POST"])
    @login_required
    @ths_sga_required
    def admin_ths_sga_toggle(identificacion):
        conn = get_db()
        record = conn.execute("SELECT activo FROM ths_sga_records WHERE identificacion = ?", (identificacion,)).fetchone()
        if record:
            nuevo_estado = 0 if record["activo"] == 1 else 1
            conn.execute("UPDATE ths_sga_records SET activo = ? WHERE identificacion = ?", (nuevo_estado, identificacion))
            conn.commit()
